...
import { getCustomer, getCustomerById, addCustomer, listCustomers, updateCustomer, saveCache, getCache, getCacheAge, deleteCache } from './database';
...
app.post('/api/chat', rateLimit(RATE_LIMIT_CHAT), async (req: Request, res: Response): Promise<void> => {
  ...
  const customer = getCustomer(siteId, apiKey);
  if (!customer) {
...
app.get('/api/scrape', rateLimit(RATE_LIMIT_SCRAPE), async (req: Request, res: Response): Promise<void> => {
  ...
  const apiKey = sanitizeString(req.query.apiKey as string, 200);
...
app.get('/api/config/:siteId', rateLimit(RATE_LIMIT_ADMIN), (req: Request, res: Response): void => {
