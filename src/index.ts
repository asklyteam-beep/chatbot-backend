...
import { getCustomer, getCustomerById, isOriginAllowed, addCustomer, listCustomers, updateCustomer, saveCache, getCache, getCacheAge, deleteCache } from './database';
import { buildFullContext, isValidUrl } from './scraper';

const app = express();

// ── Konstanten ────────────────────────────────────────────────────
const MAX_MESSAGE_LENGTH  = 500;
const MAX_CONTEXT_LENGTH  = 25_000;
...
function buildSystemPrompt(languageRule: string, botName: string): string {
...
// ── Chat Endpoint ─────────────────────────────────────────────────
app.post('/api/chat', rateLimit(RATE_LIMIT_CHAT), async (req: Request, res: Response): Promise<void> => {
  ...
  if (!customer) { res.status(401).json({ error: error || 'Unauthorized.' }); return; }
...
// ── Scrape Endpoint ───────────────────────────────────────────────
app.get('/api/scrape', rateLimit(RATE_LIMIT_SCRAPE), async (req: Request, res: Response): Promise<void> => {
  const siteId = sanitizeString(req.query.siteId as string, 50).toLowerCase();
  const origin = req.headers.origin;

  const { customer, error } = getCustomerByOrigin(siteId, origin);
  if (!customer) { res.status(401).json({ error: error || 'Unauthorized.' }); return; }

  const cacheAge = getCacheAge(siteId);
  if (cacheAge !== null && cacheAge < CACHE_TTL_HOURS) {
    const cached = getCache(siteId);
    if (cached) {
  ...
  const { valid, parsed } = isValidUrl(customer.websiteUrl);
  if (!valid || !parsed) { res.status(400).json({ error: 'Invalid website URL.' }); return; }

  try {
    console.log(`[Cache MISS] Scraping ${customer.websiteUrl} for ${siteId}...`);
    const { text, links } = await buildFullContext(customer.websiteUrl, parsed);
    saveCache(siteId, text, links);
...
app.post('/api/admin/customers', rateLimit(RATE_LIMIT_ADMIN), (req: Request, res: Response): void => {
  const adminKey = sanitizeString(req.headers['x-admin-key'] as string, 200);
  if (!validateAdminKey(adminKey)) { res.status(401).json({ error: 'Unauthorized.' }); return; }
...
app.get('/api/admin/customers', rateLimit(RATE_LIMIT_ADMIN), (req: Request, res: Response): void => {
  const adminKey = sanitizeString(req.headers['x-admin-key'] as string, 200);
  if (!validateAdminKey(adminKey)) { res.status(401).json({ error: 'Unauthorized.' }); return; }
...
app.patch('/api/admin/customers/:siteId', rateLimit(RATE_LIMIT_ADMIN), (req: Request, res: Response): void => {
  const adminKey = sanitizeString(req.headers['x-admin-key'] as string, 200);
  if (!validateAdminKey(adminKey)) { res.status(401).json({ error: 'Unauthorized.' }); return; }
  const siteId = sanitizeString(req.params.siteId, 50).toLowerCase();
  const result = updateCustomer(siteId, req.body);
  if (!result.success) { res.status(400).json({ error: result.error }); return; }
  res.json({ message: 'Customer updated successfully.' });
});

// ── Admin: Cache löschen ──────────────────────────────────────────
app.delete('/api/admin/cache/:siteId', rateLimit(RATE_LIMIT_ADMIN), (req: Request, res: Response): void => {
  const adminKey = sanitizeString(req.headers['x-admin-key'] as string, 200);
  if (!validateAdminKey(adminKey)) { res.status(401).json({ error: 'Unauthorized.' }); return; }
  const siteId = sanitizeString(req.params.siteId, 50).toLowerCase();
  deleteCache(siteId);
...
// ── Admin: Cache Status ───────────────────────────────────────────
app.get('/api/admin/cache', rateLimit(RATE_LIMIT_ADMIN), (req: Request, res: Response): void => {
  const adminKey = sanitizeString(req.headers['x-admin-key'] as string, 200);
  if (!validateAdminKey(adminKey)) { res.status(401).json({ error: 'Unauthorized.' }); return; }
  const customers = listCustomers();
  const cacheInfo = customers.map(c => {
    const age = getCacheAge(c.siteId);
